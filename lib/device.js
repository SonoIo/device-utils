...
var Device = module.exports = _.extend({}, Backbone.Events);
...
Device.getViewport = function getViewport() {
	...
	if ( Device.getOS().name == 'iOS' && Modernizr &&  Modernizr.mq('(orientation: landscape)') )
		orientation = "landscape";
	else if ( Device.getOS().name == 'iOS' && Modernizr && Modernizr.mq('(orientation: portrait)'))
...
Device.getOS = function getOS() {
	...
	// is an App test
	else if (global.window.nwDispatcher && global.window.process && global.window.process.versions['node-webkit']){
		result = {
			name: 'Desktop',
...
// Rimuove gli elementi collegati dalla classe devicw
