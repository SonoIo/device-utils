...
Device.isWindowsPhone = function isWindowsPhone(){
	if ( !Device.isMobileApp() ){
		...
		return Device.getCordova().platformId.toLocaleLowerCase() === "windowsphone";
	}
}
...
Device.onOrientationChange = function onOrientationChange(event) {
	...
	if ( os.name && os.name.toLowerCase() == 'android'  ){
		...
		var oldViewPort = _.clone(viewport);		
		if ( viewport.orientation == "portrait" ){
			viewport.orientation = "landscape";
			viewport.height = oldViewPort.width;
			viewport.width  = oldViewPort.height;
		}else{
...
Device.bind = function bind() {
	...
	$(window).bind('orientationchange', Device.onOrientationChange);

	// Finestra ridimensionata
	$(window).bind('resize', Device.onResizeViewport);
